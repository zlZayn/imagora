...
* **行动按钮体系**：两档胶囊按钮——`btn-primary` 常态实底白字、hover 白色涟漪从中心向左右展开（白底+主题字）；`btn-ghost` 透明描边、hover 主题色涟漪填充反白；`--btn-fill` 固定基础色。涟漪层为按钮同形层（`inset:0` + `scaleX(0)→1`，从中间往两边横向展开，任意宽度全覆盖）。状态类 `btn-busy`/`btn-draggable`/`btn-danger`，颜色跟随窗口主题色；画布选中操作栏即透明场景（毛玻璃 `bg-white/20` 面板 + 透明按钮）。非行动按钮（控件）不走本体系。实现与边界见 ARCHITECTURE.md 8.4。
