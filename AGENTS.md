...
* **行动按钮体系**：两档胶囊按钮（`btn-primary` 实底辉光 / `btn-ghost` 透明描边涟漪填充，`--btn-fill` 固定基础色）+ 状态类 `btn-busy`/`btn-draggable`/`btn-danger`，颜色跟随窗口主题色；画布选中操作栏即透明按钮场景（毛玻璃面板上常态透明、hover 涟漪）；非行动按钮（控件）不走本体系。实现与边界见 ARCHITECTURE.md 8.4。
