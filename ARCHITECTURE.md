...
| POST | `/api/canvas/workflow/save` | { name, nodes, edges } | { ok, path }（写 version 1 JSON 文件到固定目录 `output/workflows/`，仅需名字） |
| GET | `/api/canvas/workflow/list` | 无 | { workflows[ name, modified ] }（按修改时间倒序） |
| GET | `/api/canvas/workflow/load` | ?name= | { name, nodes, edges, missing[registryId] }（固定目录按名解析 + 文件存在性校验，缺失进 missing） |
| POST | `/api/canvas/recovery/save` | { nodes, edges } | { ok, path, name, savedAt }（独立恢复快照，不覆盖手动命名工作流） |
