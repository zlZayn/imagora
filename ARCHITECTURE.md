...
- **一屏全览（fitView 放宽）**：`minZoom` 属性降到 0.05（滚轮可缩更小），`ReactFlow fitViewOptions` 与 `Controls fitViewOptions` 显式传 `minZoom: 0.02`——`@xyflow/system` 的 `fitViewport` 用 `options.minZoom ?? minZoom` 计算视口，左下角「适应视图」按钮因此能缩到任意小，节点再多也能全览；`handleAutoLayout` 整理后同样带该 minZoom
