import type { GenerationTaskSnapshot } from "../generationQueue";
  ...
  return (
    <details className="border-y border-neutral-200 bg-white/70 px-3 py-2 text-xs">
      ...
      <div className="mt-2 grid max-h-36 gap-1 overflow-auto border-t border-neutral-100 pt-2">
        {tasks.map((task) => (
          <div key={task.id} className="flex min-h-7 items-center gap-3 px-1 text-neutral-500">
            <span className="min-w-0 flex-1 truncate font-mono">{task.id}</span>
            <span className={task.status === "failed" ? "text-red-600" : ""}>{STATUS_LABEL[task.status]}</span>
            {(task.status === "queued" || task.status === "running") && (
              <button type="button" className="text-neutral-500 hover:text-red-600" onClick={() => onCancel(task.id)}>
