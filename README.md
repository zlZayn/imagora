...
**背景**：电商商品图需要按固定结构产出（轮播图 5 张 + 详情图 5 张），
原先每款商品靠人工逐张制作，约需 **1.5 小时**；且多模态模型返回的提示词
格式不稳定，无法直接批量处理。

**做法**：做一个面向这个场景的工作台——把「参考图理解 → 提示词规范 →
...
| 图片双击放大 | 参考图 / 结果图 / 生产历史 / 画布图片均支持双击放大预览（全屏：滚轮缩放、拖拽平移，同一组件） |
| 输出目录自由 | 手输路径或文件夹选择器，生成后一键打开 |
| 批量生图 | 按产品配置一次生成多张，任务预览 + 进度条，预览不花钱 |
| 成本看板 | 生成历史面板顶部实时统计：今日/累计花费、成功率、失败数、成功平均耗时、按尺寸分布（口径为账本**原始行**，不去重不截断，与实际计费一致） |
| 预算保护 | 本机可设「当日预算」「单次上限」（0 = 不限）：提交前先按张数×尺寸预估费用，超限弹确认（明确写出超了多少），确认后才会提交；设置存在 `output/.budget.json`，不进仓库 |
| 重跑失败项 | 生成历史里失败记录可「重跑」；顶部一键重跑当前列表全部失败项（确认弹窗含费用预估与不可重跑原因统计）。参考图仍能找回的记录原样还原图生图；参考图已丢失的会明确跳过并报因，不静默降级成文生图 |
...
- 左上角「logo + Imagora」品牌区：默认平面，鼠标靠近时 3D 挤出立体并随光标摆动（移开回摆），点击在新窗口打开远程仓库
...
- 连线 = 参考关系：图片连到提示词 = 该图作为此任务参考图；连线始终显示在最上层，不被任何节点遮挡
- 类型硬约束：图片→提示词/图片组、图片组→提示词/图片组（组可中转聚合：上游图片沿组链递归传递，如 2 张组 + 3 张组 → 同一组 = 5 张）、提示词→结果；连错拒绝；提示词顶部仅一条入边；聚合按图片自动去重，含重复时组卡标「去重」
- 图片可复用：一张图可连多个提示词；画布上同一文件只有一个节点；图片下方「引用 N 处」= 数据最终流到 N 个提示词（经图片组中转到达也算，只连组未接提示词则不计）
...
- 拖拽本地图片（可多张）到画布松开即添加；新建卡片 / 图片组按钮拖出到画布松开即建（拖到画布外松手则取消，不想要随时拖走即弃；点击仍居中）
...
生成历史面板顶部即为成本看板（打开画布 →「生成历史」）：

- 指标：今日花费 / 累计花费 / 成功率（成功/总数）/ 失败数 / 成功平均耗时 / 按尺寸分布；数据来自 `/api/history/stats`，口径是账本**原始行**（每次成功都真实花过钱，聚合去重会少算，因此这里不去重）
- 预算：`日预算` 与 `单次上限` 两个输入框，单位元，**0 = 不限**（默认不限，不打扰）；保存后写入 `output/.budget.json`（git 忽略，只在本机生效）
- 生效方式：提交前（单张或批量重跑）先做一次无副作用的预检 `/api/budget/check`，超限时弹确认框写出「今日已花 + 本次预估 vs 预算」，确认后才带标记提交；服务端闸门同样校验（超限未确认返回 409），多窗口同时提交也不会漏拦

## 重跑失败项

- 入口：生成历史面板——失败行右侧「重跑」（单条），面板顶部「重跑失败项（N）」（当前列表内全部失败记录）
- 确认弹窗会列出：本次条数、预估费用、预算状态、**不可重跑的原因统计**（如「参考图已丢失 ×3」）、输出目录（可改）
- 结果：任务进入服务端并发队列，面板内显示 `重跑中 x/y`，完成后自动刷新列表与看板；产物落输出目录并写入历史，可在历史里再「导入当前画布」
- 重要：可重跑的前提是**参考图仍能从资产库找回**（`.assets` 副本，按账本 `inputAssetIds` 解析）。早于「提交时登记参考图」机制的历史失败记录（本机 8-19 之前）没有参考图记录，只能显示原因、无法重跑——重跑功能对**今后**的失败有效
