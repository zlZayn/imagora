...
* 命令行单张生图：与网页表单完全对等，所有参数显式指定、参考图可多张、产物同源可互查（详见「命令行（CLI）全量功能」）
...
uv run python -m main gen "a red apple" --size 1024x1024 --quality high -o out.png   # 单张生图（全量参数见下节）
...
CLI 单次同步等待结果（不进并发池）；需多张并行可开多个终端各跑一条 `gen` 命令。
...
* **结果回流 / 统一存储**：生成结果自动成为画布上的新图片节点，放在提示词卡片正下方，可继续连线迭代；**经典表单与画布同一套后端存储**——经典生成的结果图与参考图也会登记进 `.assets/`（资产库）注册表并落一份提交图快照，经典结果区可「导入画布」把该次生成（参考图→提示词→结果）整图还原到画布

* **核对不乱**：悬停/选中提示词节点会高亮它的所有入边与关联图片，一眼确认"这个任务带哪几张图"

* **自动整理**：选中节点后，画布右上角操作栏「自动整理」按连线深度分层、再按**提示词卡片自底向上**层层定位——图片组站到它那组卡片的上方正中央，图片站到图片组上方中央，一张图连多个组就站在各组中央上方；同参考来源的提示词卡片从左到右按左上角标题排序；结果图被复用或出现多级链路时（图→卡→结果→组→卡…任意多级）逐层向下/向上自动延伸、连线不横穿。连续点击「自动整理」结果保持不动（不会漂移），其余节点原位不变

* **保存 / 加载**：整个画布存为 JSON 工作流（`output/workflows/`），随时还原；项目文件夹改名/移动后旧工作流依然可恢复，历史版本存档自动兼容

* **入口与选中操作栏**：拖拽本地图片（可多张）到画布任意位置松开即添加，图片落在鼠标松开处；新建卡片 / 新建图片组按钮可直接拖到画布任意位置松开即建（点击仍自动居中）；工具栏上传图片 / 粘贴导入（按导入格式一键批量建卡，格式见 `docs/prompt-import-format.md`）/ 生成历史导入；右键拖拽框选 / Ctrl+点击加选后，右上角出现操作栏：运行所选、自动整理、自动连线（只补选中节点间的连线）、设置输出路径、删除所选（也可按 `Delete`）；双击连线删除；画布内右键不会弹出浏览器菜单
...
* **缩略视图（节点多不卡）**：缩小到 0.1 倍以下时节点切换为抽象卡片——图片保留缩略图与双击放大、提示词卡片只显示大字标题与居中状态（不可编辑、无运行按钮）、连线动画静止；只影响渲染，拖拽/选中/连线等操作不变，放大回 0.2 倍以上自动恢复完整卡片；画布同步启用视口虚拟化（只渲染可视区域的节点）

> **数据迁移 / 清单修复**（维护向，日常无需执行）：画布数据已版本化（v1/v2 自动兼容），需要时用独立脚本 `scripts/migrate.py`：
>
> * 查看当前存储状态（只报告，不改文件）：`python scripts/migrate.py`
>
