...
- **入口与选中操作栏**：工具栏上传图片 / 新建卡片 / 新建图片组 / 粘贴导入（契约一键批量建卡，模板见 `docs/prompt-contract.md`）/ 生成历史导入；右键拖拽框选 / Ctrl+点击加选后，右上角出现操作栏：运行所选、自动整理、设置输出路径、删除所选（也可按 `Delete`）；双击连线删除；画布内右键不会弹出浏览器菜单
