...
- **自动整理**：选中节点后，画布右上角操作栏「自动整理」按连线深度分层重排（参考图在浅层、提示词/图片组依输入下移、结果在深层；普通三段式是其特例）：整块结果在选区范围里居中（内容比选区窄时居中，比选区宽才贴着选区内侧排），不会整体偏到画面一侧；同参考来源的提示词卡片从左到右按左上角标题排序，图片直连卡片与经组卡片互不干扰、各自对齐参考图/组，一张图连多组时对称居中、不挤成左对齐；结果图被复用或出现多级链路时自动向下延伸、连线不横穿，其余节点原位不动
