...
- **入口与选中操作栏**：拖拽本地图片（可多张）到画布任意位置松开即添加，图片落在鼠标松开处；新建卡片 / 新建图片组按钮可直接拖到画布任意位置松开即建（点击仍自动居中）；工具栏上传图片 / 粘贴导入（按导入格式一键批量建卡，格式见 `docs/prompt-import-format.md`）/ 生成历史导入；右键拖拽框选 / Ctrl+点击加选后，右上角出现操作栏：运行所选、自动整理、设置输出路径、删除所选（也可按 `Delete`）；双击连线删除；画布内右键不会弹出浏览器菜单
