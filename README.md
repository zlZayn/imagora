...
- **元素**：图片节点（缩略图）+ 提示词节点（提示词 / 尺寸 / 质量 / 输出路径 / 运行按钮）
- **连线即参考关系**：把几张图片连到一条提示词节点上 = 这几张图同时作为该任务的参考图；连线只允许 图片 → 提示词（方向与类型硬约束，连错被拒绝）
- **图片可复用**：一张图可同时连多个提示词节点；同一文件画布上只出现一个节点（内容去重），复用走多条连线
- **独立任务**：每个提示词节点是独立生成任务，点「运行」只带自己的入边图片；「全部运行」并行排队执行（并发 2）
- **防重复生成**：点「全部运行」后所有待运行节点立即标记为「排队中」（橙色徽标、按钮禁用），运行中节点显示「生成中 · Ns」（每秒刷新），完成显示「完成 · N 张」；单点「运行」时双层守卫（queuedRef / runningRef），同一节点不会重复触发
