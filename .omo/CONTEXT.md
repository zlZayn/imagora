...
**画布「拖拽添加」已统一并完成结构重构：前端 tsc / lint / build 全绿、94 用例通过，E2E 31/31 PASS，git 分步提交（feat → refactor×2）。**
...
- **自动整理算法泛化（复杂连接不乱套）**：`workflow.ts:autoLayout` 从固定三段式改为**最长路径分层**（普通三段式是其特例 0/1/2/3）——结果图被复用（连图片组/别的提示词）或多级链路自动向下延伸，连线只朝下不横穿；环容忍（回边不计层）、孤立节点按类型保底（图/组 0、提示词 1）、无前驱左对齐打底（origin 模式二次整理不漂移）；新增 5 条复杂连接单测
