...
App.tsx:handleGenerate → api.ts:generateImage
  → POST /api/generate（multipart: prompt + image(可选单张) + size + quality + output_dir）
...
| POST | `/api/generate` | multipart：prompt、image(可选单张)、size、quality、output_dir | { results[status,message,url?,size,cost], messages[], totalCost } |
