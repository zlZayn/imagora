...
App.tsx:handleGenerate → api.ts:generateImage
  → POST /api/generate（multipart: prompt + images + size + quality + output_dir）
...
| POST | `/api/generate` | multipart：prompt、images(可多张)、size、quality、output_dir | { results[status,message,url?,size,cost], messages[], totalCost } |
