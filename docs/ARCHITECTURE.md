...
- **生成状态机**：提示词节点状态 `idle → queued → running → done`（前端 `status` 字段）。点「全部运行」时所有待运行节点**立即**置 `queued`（橙色徽标「排队中」、运行按钮禁用），由 worker 真正开始执行时转 `running`；`running` 徽标显示「生成中 · Ns」，由独立计时器每秒刷新 `elapsedSec`；完成显示「完成 · N 张」。状态变更通过 `setNodes` 更新对应节点 `data.status` / `data.elapsedSec` / `data.results`
